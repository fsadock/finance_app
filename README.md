...
- **Code by email (optional):** with **Configurações → Entrar por e-mail** filled in (a [Resend](https://resend.com) API key, free up to 3.000/month, and the address to receive it), the sign-in page offers "Receber código". That's what makes a first sign-in, a new device or a lost-passkey recovery possible without touching the server. The code only ever goes to the saved address, never to one typed on the page, and it's limited to 3 requests every 15 minutes.
- **Lost every passkey?** Ask for a code by email and register a new one. Without email configured, read the code from the log. Whoever reads that mailbox can get in, so use an address with its own strong password and 2FA.
